...
from pages.flight import Flight
...
@allure.parent_suite("Flight booking")
@allure.suite("Login details")
@allure.label("User details")
@allure.epic("Enter user details")
@allure.title("Ticket booking")
def test_login(driver, logger):

    logger.info("Starting the flight ticket booking.")
    logger.info("Launch flight booking page")
    driver.get("https://rahulshettyacademy.com/dropdownsPractise")
    print(driver.title)
    print(driver.current_url)
    book = Flight(driver,logger)
    book.test_book()