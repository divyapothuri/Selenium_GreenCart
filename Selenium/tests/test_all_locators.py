...
from pages.AllPractice import AllPractice
...
@allure.parent_suite("all practice")
@allure.suite("all details")
@allure.label("User details")
@allure.epic("Enter user details")
@allure.title("Practice Login")
def test_login(driver, logger):

    logger.info("Starting tt practice automation script.")
    logger.info("Launch  practice page")
    driver.get("https://rahulshettyacademy.com/AutomationPractice/")
    print(driver.title)
    print(driver.current_url)
    allp = AllPractice(driver,logger)
    allp.test_radiobutton()
    allp.test_auto_suggest_dropdown()
    allp.test_dropdown()
    allp.test_checkbox()
    allp.test_broke_links()
    allp.test_mouse_hover()
    allp.test_visible()
    allp.test_alert()
    allp.test_open_tab()
    allp.test_open_window()
    allp.test_frames()
    # allp.test_tabs()