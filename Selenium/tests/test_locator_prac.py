...
from pages.LocatorPractice import LocatorPractice
from pages.PhoneBilling import PhoneBilling
from pages.PhoneCheckout import PhoneCheckout
from pages.PhoneDashboard import PhoneDashboard
...
@allure.parent_suite("Locator Practice")
@allure.suite("Login details")
@allure.label("User details")
@allure.epic("Enter user details")
@allure.title("LocatorPractice Login")
def test_locator_practice(driver, logger):
    logger.info("Starting the Login form practice automation script.")
    logger.info("Launch locator practice page")
    driver.get("https://rahulshettyacademy.com/locatorspractice/")
    print(driver.title)
    print(driver.current_url)
    loc = LocatorPractice(driver,logger)
    loc.test_page()
    name = config['user']['user_name']
    email = config['user']['user_email']
    phonenumber = config['user']['user_phonenumber']
    loc.test_creds(name, email,phonenumber)
